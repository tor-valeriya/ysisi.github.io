...
function cartTotals(deliveryMethod = null) {
      ...
      .map((item) => {
        ...
        return `
          <article class="cart-row">
            ...
            <div>
              <h3>${product.name}</h3>
              <p>${copy.cart.size} ${item.size} · ${copy.cart.preorder}</p>
...
function buildOrderEmail(form) {
  ...
  const totals = cartTotals(data.get("delivery"));
  const lines = cart.map((item) => {
    const product = getProduct(item.productId);
    return `- ${product.name} | ${ui().cart.size}: ${item.size} | ${ui().product.quantity}: ${item.qty} | ${formatPrice(item.price * item.qty)}`;
  });
